...
use std::sync::{Arc};
use crate::v4::server::ipc::{SharedData,ThreadMessage,ExecuterMessage,WriterResponseMessage};
...
pub fn start<F,T,U>(
    data:Arc<TokioMutex<SharedData<U>>>,
    func:F,
...
async fn execute_request<F,T,U>(
    data:Arc<TokioMutex<SharedData<U>>>,
    func:F,
...
async fn process_request<F,T,U>(
    data:Arc<TokioMutex<SharedData<U>>>,
    channel_id:&str,
    r:gObjectValue,
    func:F,
    ...
    U:Unpin + Send + 'static + Sync
{
    ...
    match parse_request(r, &password){
        Ok(parsed)=>{
            match parse_request_to_struct(&channel_id,&parsed){
                Ok(request)=>{
                    ...
                    match func(data,request).await.parse(&password){
                        ...
                        Err(e)=>{
                            return Err(Error!("failed-parse_response_to_binary"=>e));
                        }
                    }
                },
                Err(e)=>{
                    return Err(Error!("failed-parse_request_to_struct"=>e));
                }
            }
        },
        Err(e)=>{
...
fn parse_request(request:gObjectValue,password:&Vec<u8>) -> Result<gObjectValue,Error>{
